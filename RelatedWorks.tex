...
Machine learning allows computers to efficiently perform specified tasks through patterns and reasoning, rather than explicit instructions. Bayesian probability models, logistic regression, decision trees, Support Vector Machines, Hidden Markov models, etc. are all standard traditional machine learning algorithms used in NLP research. Deep learning is a machine learning method based on feature learning. It transforms the original data into higher-level, more abstract feature representations through non-linear but straightforward modules. Through enough combinations of transformations, very complex functions can also be used. 

In many years of experiments, people have discovered two critical mechanisms of cognition: abstraction and iteration. From the original signal, firstly perform low-level abstraction, gradually iterate to high-level abstraction, and finally abstract higher-level patterns during iterations. In terms of deep learning, it sequentially extracts edge features, simple shape features such as the contour of the mouth, and higher-level shape features such as the face shape through a multi-layer neural network. 

In the field of natural language processing, it is not as intuitively as computer vision; we can learn the grammatical and semantic features of text information through deep learning models. we can also conclude that deep learning represents the progress of natural language processing research from machine learning to cognitive computing.

\subsection{Language Models}
To talk about adopting deep learning on NLP, we must start with the language model. The primary research of natural language processing is human-machine language interaction. Machines can understand algorithms to reflect human language. The core is the statistical language models. A language model (LM for short) is a probability distribution of a sequence of words. Through language models, the possibility of a string of text can be quantitatively assessed. For a piece of text of length n, each word in the text has the process of predicting the word through the above, and the probability product of all words can be used to evaluate the possibility of the existence of the text. In practice, if the text is long, the estimation of $P (w_i | context (w_i))$ will be difficult, so there is a simplified version: N-gram model. 
