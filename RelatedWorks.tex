...
\begin{itemize}
    ...
    \item The model is constructed with several stacked convolutional layers and pooling layers, which makes it fairly easy to build a deep network structure (considering that back to 2014, sixteen layers were considered very deep).
...
We note that on the right side of Figure \ref{fig:vgg16strucure}, the convolutional layer and pooling layer of \verb|VGG16| are divided into different blocks, these blocks are numbered \verb|Block1| to \verb|block5| from front to back. Each block includes a few convolutional layers and a pooling layer. For example: \verb|Block4| contains:
\begin{itemize}
...
Faster R-CNN \cite{fasterrcnn} lays the foundation of the two-stage method. The proposed RPN network replaces the Selective Search algorithm so that the detection task can be completed end-to-end by the neural network. Roughly speaking, Faster R-CNN is a combination of RPN plus Fast R-CNN, sharing the characteristics of convolution calculation with RCNN makes the calculation brought by RPN relatively small, so Faster R-CNN can run at 5fps on a single GPU and reaches SOTA in terms of accuracy.
