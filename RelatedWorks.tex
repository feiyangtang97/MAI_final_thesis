...
Conforming to the different sizes of the convolution kernel and numbers of convolution layers in a VGG model, there are six different \verb|ConvNet| configurations: \verb|A|, \verb|A-LRN|, \verb|B|, \verb|C|, \verb|D|, and \verb|E|, where \verb|D| and \verb|E| are more commonly discussed, named \verb|VGG16| and \verb|VGG19| respectively. Figure \ref{fig:vgg16config} displays the six different structural configurations of VGG. In Figure \ref{fig:vgg16config}, each column corresponds to a structural configuration. For example, section \verb|D| in the figure indicates the structure adopted by \verb|VGG16|.
...
\begin{itemize}
    \item 13 convolutional layers (\verb|conv3-XXX|)
    \item 3 fully connected layers(\verb|FC-XXXX|)
    \item 5 max-pooling layers (\verb|maxpool|)
\end{itemize}

Among these different layers, the convolutional layer and the fully connected layer have weight coefficients, so they are often named weighted layers. The total number of weighted layers is $13+3=16$, which represents the digit ``16'' in the name of \verb|VGG16| model. (The pooling layer does not involve weights, which means pooling layers were excluded for counting).
...
\begin{itemize}
    \item All the convolutional layers use the identical convolution kernel parameters.
    \item \verb|conv3-XXX| represents convolution layers, where \verb|conv3| indicates that the size of the convolution kernel is 3, that is, the width and height are both 3. And $3\times3$ is very small kernel size, combined with other parameters: \verb|stride = 1|, \verb|padding = same|, so that each convolutional layer (tensor) can maintain the same width and high. \verb|XXX| represents the number of channels of the convolutional layer.
    \item The pooling layer uses the same pooling kernel parameters. 
    \item \verb|VGG16| is constructed with several stacked convolutional layers and pooling layers, which makes it fairly easy to build a deep network structure (considering that back to 2014, sixteen layers were considered very deep).
    
\end{itemize}

Based on the above explanations, \verb|VGG16| becomes popular and surpasses many other models because of its small filters and deeper network architecture.
...
The convolutional layer and pooling layer of \verb|VGG16| are divided into different blocks on the right side of Figure \ref{fig:vgg16strucure}. These blocks are labelled \verb|Block1| to \verb|block5| from front to back. Each block includes a few convolutional layers and a pooling layer. For example: \verb|Block4| contains:
\begin{itemize}
...
R-CNN achieves object detection in two stages. The first is to propose a few regions of interest which may have potential objects, that is, the local cropping of the image, named the step of ``Region Proposal'' \cite{rcnn}. The paper \cite{rcnn} used selective search algorithm to run the SOTA performing classification network, i.e. AlexNet in many cases, and then obtained the types of objects in each region. A basic structure of R-CNN is illustrated below as Figure \ref{fig:rcnn}.
