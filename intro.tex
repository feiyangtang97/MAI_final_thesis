...
%% SHURONG:change this sentence to: this alignment can be evaluated by to tasks: image annotation and image search. Or you can change it to: this alignment can be used to two downstream tasks:..[done]
...
compressed \verb|jpeg| image file size ranges between 20-300 KB. The paragraph-level descriptions are split into multiple sentences and a maximum of five sentences are retained for each artwork to reduce data imbalance. The number is the accession number of an artifact that obviously cannot be derived from the input image or artwork type. We also remove duplicate images in the captioning datasets based on their hash code. Tokens occurring less than two times are removed from the training vocabulary.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{egyptian.pdf}
...
\begin{figure}[h!]
\centering
\includegraphics[width=.6\textwidth]{chinese.pdf}
...
We train our multi-modal machine learning model on the known image features and corresponding textual attributes; this training process helps us learn the potential relationships between image and text. This trained model will be used to generate textual attributes from known image features and vice versa. We believe this may help with automating the artwork annotation process and significantly save labours on on manually annotate artwork information.
