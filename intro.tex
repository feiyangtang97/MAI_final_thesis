...
As mentioned above, our primary research task here is to achieve cross-modal retrieval (i.e. between image and text). Figure \ref{fig:framework} illustrates a brief working framework for the tasks.
...
\begin{enumerate}
    ...
    \item By focusing on fragment level image features and textual attributes instead of feeding the whole images and sentences, we are able to perform cross-modal retrieval in a more fine-grained level. This allows the future multimodal retrieval tasks on artworks to achieve more accurate and stable results. (Section~\ref{cha:Method})
\end{enumerate}
